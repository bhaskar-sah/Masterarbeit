...
class PandaPushEnv(gym.Env):
    ...
    def _get_reward(self):
        ...
        push_dir, _ = self._compute_push_direction()
        ...
        # 3. Total distance (for success check)
        ...
        if self.episode_length % 3 == 0:
            ...
            print(f"Dist XY:       {dist_xy:.4f} m")
            print(f"Dist Z:        {dist_z:.4f} m")
            print(f"Dist Total:    {dist_total:.4f} m")

            print("-" * 60)

            print(f"Hand X-axis:   [{hand_x_axis[0]:.3f}, {hand_x_axis[1]:.3f}, {hand_x_axis[2]:.3f}]")
            print(f"Hand Z-axis:   [{hand_z_axis[0]:.3f}, {hand_z_axis[1]:.3f}, {hand_z_axis[2]:.3f}]")
            print(f"Push dir:      [{push_dir[0]:.3f}, {push_dir[1]:.3f}, 0.000]")
            ...
            print(f"  R_approach_xy:  {5.0 * reward_approach_xy:+.4f}  (raw: {reward_approach_xy:.4f})")
            print(f"  R_approach_z:   {5.0 * reward_approach_z:+.4f}  (raw: {reward_approach_z:.4f})")
            print(f"  R_close:        {3.0 * reward_close:+.4f}  (raw: {reward_close:.4f})")
            print(f"  R_orient_z:     {2.0 * orientation_z_reward:+.4f}")
            print(f"  R_orient_x:     {2.0 * orientation_x_reward:+.4f}")
            print(f"  R_floor:        {height_penalty:+.4f}")
