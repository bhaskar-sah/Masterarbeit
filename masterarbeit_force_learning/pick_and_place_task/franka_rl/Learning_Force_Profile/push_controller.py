"""
push_controller.py  (audited and fixed)
...
class PushController:
    ...
    def compute_torque(self, action, tilt=None):
        ...
        v_des = np.array([action[0] * self.v_max,
                          action[1] * self.v_max, 0.0])
        wz_cmd = action[2] * self.w_max
        f_modulation = (action[3] + 1.0) / 2.0
        f_magnitude = self.F_FLOOR + f_modulation * (self.f_max - self.F_FLOOR)
        ...
        # self.p_des[2] = np.clip(self.p_des[2], 0.83, 0.92)
        # self.p_des[2] = np.clip(self.p_des[2], 0.83, 0.84)
        self.p_des[2] = 0.84


        # 3. INTEGRATE wz -> yaw_des, with WRAPPING to [-pi, pi]
        ...
        # Sign correction: MuJoCo gives bottle-on-robot reaction
        F_measured_raw = self.contact_manager.get_contact_force()
        F_robot_on_bottle = -F_measured_raw

        # 5. F_des along flange axis
        ...
        if norm > 1e-6:
            ...
            flange_axis_world = np.zeros(3)
        F_des = f_magnitude * flange_axis_world

        # # 6. POSITION + FORCE CONTROL with P_ERROR CAP
        # p_error = self.p_des - p_current
        # # Cap p_error magnitude so position term doesn't dominate F_cmd
        # p_error_mag = np.linalg.norm(p_error)
        # if p_error_mag > P_ERROR_MAX:
        #     p_error = p_error * (P_ERROR_MAX / p_error_mag)

        # v_error = v_des - v_current

        # f_meas_scalar = float(np.dot(F_robot_on_bottle, flange_axis_world))
        # F_meas_projected = f_meas_scalar * flange_axis_world
        # F_error = F_des - F_meas_projected

        # F_cmd = self.Kp * p_error + self.Kd * v_error + F_des + self.Kf * F_error
        ...
        # =======================================================================================
        # 6. POSITION + FORCE CONTROL with XY P_ERROR CAP
        # =======================================================================================
        p_error = self.p_des - p_current
        
        # Cap ONLY the XY error magnitude so position term doesn't dominate F_cmd.
        # Leave the Z-axis raw so the arm doesn't float into the air!
        xy_error = p_error[:2]
        xy_error_mag = np.linalg.norm(xy_error)
        
        if xy_error_mag > P_ERROR_MAX:
            xy_error = xy_error * (P_ERROR_MAX / xy_error_mag)
            
        # Recombine the capped XY error with the raw, uncapped Z error
        ...
        # IMPORTANT: Use p_error_capped here!
        F_cmd = self.Kp * p_error_capped + self.Kd * v_error + F_des + self.Kf * F_error
        ...
        self.last_F_des = F_des.copy()
        self.last_f_magnitude = f_magnitude
