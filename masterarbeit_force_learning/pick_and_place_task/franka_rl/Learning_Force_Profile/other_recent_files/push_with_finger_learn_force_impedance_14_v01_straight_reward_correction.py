...
class PandaPushTrajectoryEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, trajectory_type="straight"):
        ...
        # added new for wrong body contact detection
        ...
        self.lookahead_points = 8 # 4  # Look 4 points ahead on trajectory
        ...
        print(f"\n{'=' * 60}")
        print("LOOKAHEAD TARGET + Learned Wrist + Wrong body detection")
        print(f"{'=' * 60}")
    ...
    def _compute_push_velocity(self, action):
        ...
        current_target_z = self.target_z + 0.02 * distance_from_start # was 0.02
        ...
        if self.is_settling:
            ...
            if self.is_settling:
                ...
                v_desired[2] = self.z_gain * (current_target_z - hand_pos[2]) # current_target_z was self.target_z
                return v_desired
        ...
        # # ==================== WRONG BODY CONTACT ====================
        # if self._is_wrong_body_touching():
        #     # Back away to break unwanted link6/link7 contact
        #     bottle_direction = bottle_xy - hand_pos[:2]
        #     bottle_dist = np.linalg.norm(bottle_direction)
        #     if bottle_dist > 0.001:
        #         away_dir = -bottle_direction / bottle_dist
        #         v_desired[0] = away_dir[0] * 0.03
        #         v_desired[1] = away_dir[1] * 0.03
        #         v_desired[2] = self.z_gain * (self.target_z - hand_pos[2])
        #         if self.episode_length % 50 == 0:
        #             print(f"    WRONG BODY CONTACT! Backing away")
        #         return v_desired
        ...
        v_tracking = np.clip(v_tracking, -0.05, 0.05) # was 0.03
    ...
    def _is_wrong_body_touching(self):
        """Check if link6 or link7 is touching the bottle (bad contact)."""
        for i in range(self.data.ncon):
            contact = self.data.contact[i]
            body1 = self.model.geom_bodyid[contact.geom1]
            body2 = self.model.geom_bodyid[contact.geom2]
    ...
    def _get_reward(self, action=None):
        ...
        if self.in_approach:
            ...
            r_progress = 50.0 * max(progress_delta, 0) # was # 100.0 * max(progress_delta, 0)
            ...
            # make the r_deviation quadratic
            ...
            #============= new continuous scaling) ==============
            # Maps 0 degrees to +2.0, 30 degrees to 0.0, and >=60 degrees to -2.0
            # r_angle = np.clip(2.0 - (angle_deg / 15.0), -2.0, 2.0)
            if not is_touching or force_mag < 0.5:
                r_angle = 0.0  # No angle reward when not in contact
            else:
                # this is the design choics and I choose 60°. Beyond 60° misalignment, the force is so poorly directed that we
                # do not care how much it gets. It is equally bad
                angle_deg_clamped = min(angle_deg, 60.0) # Beyond 60° is equally bad
                r_angle = 2.0 - (angle_deg_clamped / 15.0)
                ######################
                # r_angle = np.clip(2.0 - (angle_deg / 15.0), -2.0, 2.0)
            ...
            if wrong_body:
                r_contact = -5.0 # Link6/7 touching bottle - bad configuration
            elif is_touching:
                r_contact = 2.0
            else:
            ...
            # 6. Action Smoothing Penalty (NEW: penalize jittering stiffness)
            if action is not None and self.prev_action is not None:
                # Calculate how much the stiffness actions (indices 2 and 3) changed
                k_delta = np.linalg.norm(action[2:4] - self.prev_action[2:4])
                r_smooth = -0.1 * k_delta  # Penalty multiplier. Adjust if still jittery! before -0.5 ( basically to high)
