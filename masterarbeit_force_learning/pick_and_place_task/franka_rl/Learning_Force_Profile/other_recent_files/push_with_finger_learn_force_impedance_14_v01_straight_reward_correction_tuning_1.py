"""
Panda Push Environment - More RL-Based Version

Main idea:
    - Keep only a simple nominal pushing prior
    - RL learns lateral correction through actions and reward
    - action = [forward_mod, lateral_mod, Kx, Ky]
    - wrist is automatically aligned with desired push direction
    - reward is smooth and interpretable

Compared to the previous manual version:
    - Removed hand-coded tangent/correction blend as main decision logic
    - Removed RL wrist action
    - Reduced manual correction strategy
    - Kept only simple approach, recovery, and safety logic
"""
...
class PandaPushTrajectoryEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, trajectory_type="straight"):
        ...
        self.lateral_push_scale = 0.45 # 0.30
        self.lateral_hand_offset_scale = 0.04 # 0.03
        ...
        print(f"{'=' * 60}")
        print("Nominal prior: push along path tangent")
        print("RL controls: forward speed, lateral correction, Kx, Ky")
        print("Wrist is automatically aligned to desired push direction")
    ...
    def _get_lookahead_target(self, bottle_xy):
        """
        Returns a point ahead on trajectory (index + N)
        """
        if self.trajectory is None or len(self.trajectory) < 2:
        ...
        lookahead_steps = 5  # important hyperparameter (test with different values 3–8)
...
##############################################################################################
    ...
    #     """
    #     Nominal direction is the path tangent.
    #     RL chooses how much lateral correction to add using the deviation direction.
    #     """
    ...
    #         correction_dir = deviation_vec / deviation_mag  # from bottle to trajectory
    #     else:
    #         correction_dir = np.zeros(2, dtype=np.float32)
    #
    #     raw_dir = tangent_dir + self.lateral_push_scale * lateral_mod * correction_dir
    ...
    #
    #     # Blend local tangent with future path direction
    #     blend = 0.5  # later you need to try 0.3 to 0.7 values between them.
    #
    ...
    #         nominal_dir = tangent
    #
    #     # Lateral direction perpendicular to nominal_dir
    ...
    def _get_rl_push_direction(self, bottle_xy, lateral_mod):
        """
        Use lookahead-based direction as nominal (handles curves automatically).
        RL adds lateral correction perpendicular to this direction.
        """
        # Lookahead direction as nominal — automatically handles curves
        lookahead_dir, _ = self._get_lookahead_direction(bottle_xy)
        tangent = self._get_path_tangent(bottle_xy)
        deviation_vec, deviation_mag = self._get_path_deviation(bottle_xy)

        # Blend: on-track = tangent, off-track = lookahead correction
        ...
        # RL lateral correction perpendicular to nominal
        lateral_dir = np.array([-nominal_dir[1], nominal_dir[0]], dtype=np.float32)
        push_dir = nominal_dir + lateral_mod * lateral_dir * 0.3
...
##############################################################################################
    def _get_hand_target_position(self, bottle_xy, lateral_mod):
        """
        Nominal hand target:
            - behind bottle along tangent
            - RL chooses lateral offset
        """
        tangent_dir = self._get_path_tangent(bottle_xy)
        deviation_vec, deviation_mag = self._get_path_deviation(bottle_xy)
...
# ===============0 added contact body touch again ===========================
    def _is_wrong_body_touching(self):
        for i in range(self.data.ncon):
            ...
            if bottle_touch:
                ...
                if other_body == self.link6_body_id or other_body == self.link7_body_id:
                    return True
        return False
    ...
    def _check_stable(self):
        ...
        return tilt > 0.97 and angvel < 0.1 # was 0.99
    ...
    def _compute_push_velocity(self, action):
        """
        More RL-based push controller:
            - RL controls forward speed and lateral correction
            - nominal prior is path tangent
            - minimal recovery / safety logic remains
        """
        hand_pos = self.data.xpos[self.hand_body_id]
        bottle_xy = self.data.xpos[self.bottle_body_id][:2]
        tilt = self._get_bottle_tilt()
...
##################################################################################
    ...
    def _get_reward(self):
        ...
        if self.in_approach:
            ...
            r_deviation = -w_deviation * deviation_mag * deviation_mag * 10.0
    ...
    def step(self, action):
        ...
        if self.in_approach:
            ...
            # Pre-align wrist with trajectory tangent during approach
            bottle_xy = self.data.xpos[self.bottle_body_id][:2]
            tangent_dir = self._get_path_tangent(bottle_xy)
            push_angle = np.arctan2(tangent_dir[1], tangent_dir[0])
            alignment_rotation = push_angle - self.gripper_push_angle_at_home
            while alignment_rotation > np.pi:
                alignment_rotation -= 2 * np.pi
            while alignment_rotation < -np.pi:
                alignment_rotation += 2 * np.pi
            target_wrist_rotation = np.clip(
                alignment_rotation,
            ...
            # Wrist aligns automatically with desired push direction
            push_dir = self.last_desired_push_dir
