...
USER'S CONCEPT (from drawing):
- Hand always stays behind bottle
- A "direction vector" always points from bottle towards trajectory
- Push force = Forward (along path) + Correction (towards path)
- Correction strength is proportional to deviation

This is ONE CONTINUOUS BEHAVIOR, not separate modes!

    Trajectory ═══════════════════════════════════════
                        ↑
                        │ correction_vector (always present!)
                        │ magnitude = deviation distance
                       🍾 Bottle
                        ↑
                        │ forward_vector (along tangent)
                      ┌───┐
                      │🤖│ Hand: pushes with forward + correction
                      └───┘

    Total push = forward_vector + K * correction_vector
...
class PandaPushTrajectoryEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, trajectory_type="straight"):
        ...
        # ==================== PUSH PARAMETERS ====================
        self.base_forward_speed = 0.015 # 0.008  # Original working value
        self.behind_distance = 0.04  # Distance behind bottle

        # -- Fixed Y-Axis of the hand --
        # After contact, hand y-axis stays fixed and only x-axis moves for correction
        self.fixed_y = None # will be set when contact is made
        self.fixed_y_active = False # flag to activate fixed y-axis mode
        ...
        self.base_correction_gain = 0.25  # Reduced to prevent tipping!
        ...
        # Cartesian control
        # target_z is for HAND FRAME (Jacobian controls this)
        # Gripper center is ~0.1m below hand frame
        #
        # Bottle geometry (from XML):
        #   - Table surface: z = 0.8
        #   - Bottle base: z = 0.805
        #   - Bottle center: z = 0.805 + 0.09 = 0.895
        #   - Bottle top: z = 0.805 + 0.18 = 0.985
        #
        # For gripper to contact bottle center (z=0.895):
        #   hand_z = gripper_z + 0.1 = 0.895 + 0.1 = 0.995
        #
        # Slightly lower to push below center for stability:
        self.target_z = 0.92  # Hand frame height → gripper at ~0.88m
        self.z_gain = 10.0
        self.damping = 0.01
        ...
        print(f"{'=' * 60}")
        print("DIRECTION VECTOR CONCEPT + FIXED Y")
        print(f"{'=' * 60}")
        print("After contact: Hand Y is FIXED, only X moves for correction")
        print(f"K range: [{self.K_min}, {self.K_max}] N/m")
        print(f"{'=' * 60}")
    ...
    def _get_correction_vector(self, bottle_xy):
        ...
        This vector is ALWAYS present, even when deviation is small!
        It acts like a spring pulling the bottle back to the path.
        """
    ...
    def _get_hand_target_position(self, bottle_xy):
        """
        Hand stays BEHIND the bottle.
        "Behind" means opposite to the combined push direction.
        """
        forward_vec = self._get_forward_vector(bottle_xy)
        correction_vec, correction_mag = self._get_correction_vector(bottle_xy)

        # Combined push direction (normalized)
        # Weight correction by stiffness
        K_normalized = np.mean(self.current_K) / self.K_max

        if correction_mag > 0.001:
            correction_dir = correction_vec / correction_mag
            # Blend: more correction when deviation is large
            correction_weight = min(correction_mag * 10 * K_normalized, 0.6)
        ...
        # Hand position: behind bottle relative to push direction
    ...
    def _compute_push_velocity(self, action):
        ...
        # 1. FORWARD COMPONENT: Along trajectory tangent (FULL 2D vector!)
        forward_speed = self.base_forward_speed * (1.0 + forward_mod * 0.3) * speed_mult
        v_forward = forward_vec * forward_speed  # Both X and Y components!

        # 2. CORRECTION COMPONENT: Towards trajectory (FULL 2D vector!)
        if correction_mag > 0.001:
            correction_dir = correction_vec / correction_mag
            ...
            # Reduce correction when tilting
            if tilt < 0.99:
                tilt_factor = max((tilt - 0.95) / (0.99 - 0.95), 0.1)
                correction_speed *= tilt_factor
            else:
                tilt_factor = 1.0

            correction_speed = min(correction_speed, 0.020)

            v_correction = correction_dir * correction_speed  # Both X and Y!
        ...
        # 3. STAY BEHIND BOTTLE
        target_pos = self._get_hand_target_position(bottle_xy)
        pos_error = target_pos[:2] - hand_pos[:2]
        v_tracking = pos_error * 2.0
        v_tracking = np.clip(v_tracking, -0.02, 0.02)
    ...
    def _compute_wrist_rotation(self, bottle_xy):
        ...
        - Bottle LEFT of trajectory  → Hand goes RIGHT → Wrist rotates POSITIVE
        - Bottle RIGHT of trajectory → Hand goes LEFT  → Wrist rotates NEGATIVE
        ...
        # correction_vec[0] > 0 means bottle is LEFT of trajectory (needs to go RIGHT)
        # correction_vec[0] < 0 means bottle is RIGHT of trajectory (needs to go LEFT)
        #
        # Hand should go to OPPOSITE side to push bottle back:
        # Bottle LEFT  → Hand goes RIGHT → POSITIVE rotation
        # Bottle RIGHT → Hand goes LEFT  → NEGATIVE rotation
        #
        # So: target_rotation = +correction_vec[0] (SAME sign, not opposite!)
        target_rotation = correction_vec[0] * K_factor * self.wrist_rotation_gain

        # also add a minimum rotation when deviation exists
        min_rotation = 0.05 if deviation_mag > 0.005 else 0.0
        if target_rotation > 0:
            target_rotation = max(target_rotation, min_rotation)
        elif target_rotation < 0:
            target_rotation = min(target_rotation, -min_rotation)
    ...
    def _get_path_deviation(self, bottle_xy):
        """Returns vector FROM bottle TO trajectory, and its magnitude."""
    ...
    def _get_obs(self):
        ...
        # Direction vectors (the key concept!)
        forward_vec = self._get_forward_vector(bottle_xy)
        correction_vec, correction_mag = self._get_correction_vector(bottle_xy)
        ...
        obs = np.concatenate([
            ...
            dir_to_bottle,  # 2
            [dist_to_bottle],  # 1
            correction_vec,  # 2 (direction to trajectory)
            [correction_mag],  # 1 (deviation magnitude)
            tangent,  # 2
            [progress],  # 1
            contact_force,  # 3
            is_touching,  # 1
    ...
    def _get_reward(self):
        ...
        if self.in_approach:
            ...
            # Deviation reward - LINEAR function for continuous feedback!
            # This gives smooth gradient instead of discrete steps
            # dev = 0cm → +5.0
            # dev = 5cm → 0.0
            # dev = 10cm → -5.0
            # dev = 15cm → -10.0
            ...
            # Print status with correction info
            if self.episode_length % 50 == 0:
                K_avg = np.mean(self.current_K)
    ...
    def reset(self, seed=None, options=None):
        ...
        print(f"Push = Forward + K * Correction")
        print(f"{'=' * 50}")
