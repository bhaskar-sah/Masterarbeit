...
class PandaPushTrajectoryEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, trajectory_type="straight"):
        ...
        self.base_correction_gain = 0.5  # Increased from 0.25 # Base correction strength
        self.k_correction_gain = 0.6  # Increased from 0.35 # Additional correction from k
        self.max_correction_speed = 0.035  # Increased from 0.020 # max correction velocity
    ...
    def _compute_push_velocity(self, action):
        ...
        v_tracking = pos_error * 2.5  # Increased from 2.0
        v_tracking = np.clip(v_tracking, -0.025, 0.025)  # Increased from 0.02
    ...
    def _generate_trajectory(self, bottle_start_xy, traj_type):
        ...
        if traj_type == "straight":
            ...
            x = start[0] + 0.08 * np.sin(2 * np.pi * t) * (1-t) # Fades out
            y = start[1] + t * (goal[1] - start[1]) # Linear Y interpolation
            return np.stack([x, y], axis=1).astype(np.float32)
