...
class PandaPushTrajectoryEnv(gym.Env):
    ...
    def _get_push_direction(self, bottle_xy):
        ...
        if deviation_mag < 0.01:
            ...
            correction_weight = min(deviation_mag * 8, 0.7)
            correction_dir = deviation_vec / (deviation_mag + 1e-6)
            push_dir = (1 - correction_weight) * tangent + correction_weight * correction_dir
            return push_dir / (np.linalg.norm(push_dir) + 1e-6)

    def _get_hand_target_position(self, bottle_xy, push_state):
        """
        Get where hand should be.

        Depends on push_state:
        - 'push', 'slow': Behind bottle, ready to push
        - 'stop', 'settle': Maintain position, don't push
        - 'backup': Move slightly away from bottle
        """
        push_dir = self._get_push_direction(bottle_xy)

        if push_state == 'backup':
            # Back off - move further from bottle
            distance = self.behind_distance + 0.02  # Extra 2cm
        elif push_state in ['stop', 'settle']:
            # Stay where we are (slightly behind)
            distance = self.behind_distance + 0.01
        else:
            # Normal pushing position
    ...
    def _compute_push_velocity(self, action, push_state):
        ...
        if push_state == 'push':
            ...
            # SLOW PUSH: Same as push but slower
            pos_error = target_hand[:2] - hand_pos[:2]
            v_reposition = pos_error * 1.5
            v_reposition = np.clip(v_reposition, -0.015, 0.015)
    ...
    def get_learned_force_profile(self):
        return {
