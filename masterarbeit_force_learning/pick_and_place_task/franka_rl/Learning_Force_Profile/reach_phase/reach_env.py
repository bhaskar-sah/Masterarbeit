...
# Path setup so the local imports below resolve regardless of which
...
class PandaReachEnv(gym.Env):
    ...
    def _load_model(self):
        ...
        # Search the reach_phase folder first, then the parent
        # Learning_Force_Profile folder where the original XML lives.
        candidate_paths = [
            Path(__file__).parent / "robot_panda_push_force.xml",
            Path(__file__).parent.parent / "robot_panda_push_force.xml",
        ]
        model_path = None
        ...
        if model_path is None:
            raise FileNotFoundError(
                f"Model not found in any of: {[str(p) for p in candidate_paths]}"
            )
